...
module MssqlKissmetrics
    ...
    def self.initialize(profile, username, password, km_key, allowed_history_days = 0)
        ...
        dbh.select_all("SELECT a.entity_id, DATE_ADD(a.created_at, INTERVAL -7 HOUR) AS created_at, a.customer_email, a.customer_firstname, a.customer_lastname, a.increment_id, a.grand_total, a.subtotal, b.region, b.postcode, b.city, a.coupon_code
                                   ...
                                   (@allowed_history_days > 0 ? "WHERE  " << @now << " - UNIX_TIMESTAMP(DATE_ADD(a.created_at, INTERVAL -7 HOUR) ) <= " << (@allowed_history_days * 86000) : "") <<
                                   "ORDER BY increment_id DESC") do |row|
            ...
            KM.record('Purchased', {'Order ID' => row['increment_id'],
                                                      'Order Total' => row['grand_total'],
                                                      'Order Subtotal' => row['grand_total'],
                                                      'Ship to Province' => row['region'],
                                                      'Coupon Code' => row['coupon_code'],
                                                      '_d' => 1,
                                                      '_t' => ts})

            dbh.select_all("SELECT b.sku, k.value AS brand, d.value AS style, j.value AS season, i.value AS product, n.value AS color, o.value AS size, (SELECT MAX(price) FROM sales_flat_order_item WHERE order_id = b.order_id AND sku = b.sku) AS price
                                       FROM sales_flat_order AS a
                                       ...
                                       WHERE a.entity_id = " << row['entity_id']) do |item|
